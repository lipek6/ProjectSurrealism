...
@onready var state_machine        : MovementStateMachine     = %MovementStateMachine
...
func _handle_headbob(delta : float) -> void:
	...
	# Duck Typing: Check if the active state inherits from GroundState!
	var is_moving_on_ground : bool = state_machine.current_state is GroundState
	
	if is_moving_on_ground and player.velocity.length() > 0.1:                  
		...
		if smooth_headbob:
			fp_camera.transform.origin = fp_camera.transform.origin.lerp(Vector3.ZERO, 10.0 * delta)

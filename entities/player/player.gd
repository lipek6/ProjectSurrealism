...
func _unhandled_input(event: InputEvent) -> void:
	...
	if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED and is_active:
		if event is InputEventMouseMotion:
			if camera_controller: camera_controller.handle_camera_input(event)
...
func _process(delta: float) -> void:
	...
	if is_active:
		if camera_controller: camera_controller.handle_controller_look_input(delta)
...
func _update_debug_ui() -> void:
	...
	if movement_state_machine.current_state:
		debug_label.text += "STATE: " + str(movement_state_machine.current_state.name)             + "\n"
	else:
		debug_label.text += "STATE: NULL\n"
		
	debug_label.text += "CAM_STATE: " + str(PlayerCameraController.Style.keys()[camera_controller.current_style]) + "\n"
	
	if camera_controller.current_camera:

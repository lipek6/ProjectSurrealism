...
enum State { IDLE, SURPRISED, CHASE, ATTACK, HIT_REACTION, FLEE, CRAWL, CRAWL_FLEE }

@export_group("Brain Settings")
@export var target_player   : Player
@export var attack_range    : float = 1.8
@export var attack_cooldown : float = 1.5
@export var max_flee_distance : float = 15.0                    # NEW: How far they run before stopping!

@export_group("Senses")
@export var sight_range       : float = 20.0
@export var field_of_view     : float = 90.0                            
@export var proximity_hearing : float = 4.0                             
...
func trigger_hit_reaction(part_name: String) -> void:
	if current_state == State.FLEE or current_state == State.CRAWL_FLEE or body.is_dead: return
	if body.is_crawling: return
...
func _get_next_state() -> State:
	...
	if body.missing_left_leg and body.missing_right_leg:
		return State.CRAWL_FLEE
		
	if body.missing_left_arm and body.missing_right_arm:
		return State.FLEE
...
func _enter_state(new_state: State) -> void:
	match new_state:
		...
		State.SURPRISED:
			_animation_locked = true
			_failsafe_timer = 0.0
			body.anim_player.play("ua1/Celebration", 0.1)
		State.CHASE:
			body.anim_player.play("ua1/Sprint", 0.2)
		State.CRAWL:
			body.anim_player.play("ua1/Crawl_Fwd", 0.3)
		State.FLEE:
			body.anim_player.play("ua2/Zombie_Run_Bwd", 0.2)
		State.CRAWL_FLEE:
			body.anim_player.play("ua1/Crawl_Bwd", 0.3)
		State.ATTACK:
			_animation_locked = true
			_failsafe_timer = 0.0
			...
			if not body.missing_right_arm:
				body.anim_player.play("ua1/Punch_Cross", 0.1) 
			elif not body.missing_left_arm:
				body.anim_player.play("ua1/Punch_Jab", 0.1) 
			else:
...
func _process_state(_delta: float) -> void:
	match current_state:
		...
		State.FLEE, State.CRAWL_FLEE:
			# --- NEW: BOUNDED FLEEING LOGIC ---
			var distance_to_player = body.global_position.distance_to(target_player.global_position)
			
			if distance_to_player > max_flee_distance:
				# They are far enough away. Stop running and just stare.
				_apply_friction()
				_look_at_flat(target_player.global_position)
			else:
				# Keep running away!
				var flee_dir = body.global_position.direction_to(target_player.global_position) * -10.0
				var flee_pos = body.global_position + flee_dir
				_navigate_towards(flee_pos)
